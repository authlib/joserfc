...
class TestJWTClaims(TestCase):
    ...
    def test_int_claims(self):
        now = int(time.time())
        claims_requests = jwt.JWTClaimsRequests(now=now)
    ...
    def test_validate_aud(self):
        claims_requests = jwt.JWTClaimsRequests(aud={"essential": True, "value": "a"})
        ...
        # use option values
        claims_requests = jwt.JWTClaimsRequests(aud={"essential": True, "values": ["a", "b"]})
        ...
        # do not validate
        claims_requests = jwt.JWTClaimsRequests()
        claims_requests.validate({"aud": "a"})

        claims_requests = jwt.JWTClaimsRequests(aud={"essential": True})
        claims_requests.validate({"aud": "a"})
